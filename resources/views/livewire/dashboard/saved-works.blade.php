@section('title',trans('frontend.my_works'))
<div id="dashboard" role="tabpanel" aria-labelledby="dashboard-tab" class="">
    <div class="bg-white p-2 rounded flex justify-between items-center  ">
        <h2 class=" px-6 text-md">{{trans('frontend.saved_works')}}</h2>
        <a href="" class="md-button primary-btn">{{trans('frontend.browse_works')}}</a>
    </div>
    <div class="w-full">
                ...
                <div class="flex-1">
                    <div class="flex items-center justify-between"
                        ...
                        <a href="" class="text-smd md:text-base  text-primaryTextColorDarken">
                           {{\Illuminate\Support\Str::limit($work->title,80)}}
                        </a>
                    ...
                    <p class="text-ellipsis overflow-hidden text-sm my-3 leading-8">
                        <span class="block">{{\Illuminate\Support\Str::limit($work->description,200)}}  </span>
                    </p>
                    <div class="grid grid-cols-2 md:flex items-center gap-1 gap-y-2 justify-start gap-x-3 p-1 border">
                        ...
                        <div class="flex items-center gap-1 text-sm ">
                            ...
                            <svg class="h-4 w-4 @if($work->timeline_point =='pending') text-yellow-500
                                @else text-green-500 @endif"
                                 fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <span>{{trans('frontend.projects.'.$work->timeline_point)}}</span>
                        </div>
                        <div class="flex items-center gap-1 text-sm ">
                            <svg class="h-4 w-4 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @empty
        <x-alert type="info" message="{{trans('frontend.no_item_added_yet',['title'=>trans('frontend.work')])}}" />
        @endforelse
